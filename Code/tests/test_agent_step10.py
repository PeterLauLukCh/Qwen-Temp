...
class AgentLoopTest(unittest.TestCase):
    ...
    def test_incomplete_transient_interconnection_request_uses_cia_readiness_gate(self) -> None:
        llm = FakeLLMClient([])
        agent = GridMindAgent(registry=FakeRegistry(), llm_client=llm)
        ...
        self.assertEqual(result.tool_records, [])
        self.assertIsNotNone(result.readiness_check)
        self.assertEqual(result.readiness_check["type"], "cia_readiness")
        self.assertIn("case", result.output_text)

    def test_complete_joint_transient_interconnection_request_reaches_llm(self) -> None:
        llm = FakeLLMClient([final_response("I should call run_integrated_assessment.")])
        agent = GridMindAgent(registry=FakeRegistry(), llm_client=llm)

        result = agent.run_turn(
            "Can we add a 5 MW solar project at bus 10 on IEEE 14 and survive "
            "a bus fault at bus 2 from 1.0 s to 1.1 s?"
        )
        ...
        self.assertIn("run_integrated_assessment", result.output_text)

    def test_joint_transient_request_missing_connection_bus_stops_before_llm(self) -> None:
        llm = FakeLLMClient([final_response("Do not use me.")])
        agent = GridMindAgent(registry=FakeRegistry(), llm_client=llm)
        ...
        self.assertEqual(len(llm.calls), 0)
        self.assertEqual(result.tool_records, [])
        self.assertIn("target bus", result.output_text)

    def test_joint_transient_request_missing_fault_timing_stops_before_llm(self) -> None:
        llm = FakeLLMClient([final_response("Do not use me.")])
        agent = GridMindAgent(registry=FakeRegistry(), llm_client=llm)
        ...
        self.assertEqual(len(llm.calls), 0)
        self.assertEqual(result.tool_records, [])
        self.assertIn("fault", result.output_text)
        self.assertIn("time", result.output_text)

    def test_joint_load_acceptance_transient_prompt_reaches_llm(self) -> None:
        llm = FakeLLMClient([final_response("Use run_integrated_assessment.")])
        agent = GridMindAgent(registry=FakeRegistry(), llm_client=llm)
