...
class AndesPandaPowerSolver(PandaPowerSolver):
    """pandapower solver initialized from a public ANDES RAW-derived case.

    This adapter is intentionally narrow.  It exists for strict integrated
    IEEE118 M1+M2 studies where the M1 steady-state network must originate from
    the same public RAW data used by the ANDES dynamic stage.
    """

    _SUPPORTED_CASES = ("ieee118",)
    _CASE_ALIASES = {
        "ieee118": "ieee118",
        "case118": "ieee118",
        "118": "ieee118",
        "ieee118dynamic": "ieee118",
        "ieee118public": "ieee118",
        "ieee118publicdynamic": "ieee118",
    }

    def __init__(
        self,
        use_numba: bool = False,
        case_files: Optional[PublicDynamicCaseFiles] = None,
    ) -> None:
        super().__init__(use_numba=use_numba)
        self.case_source_metadata: Dict[str, Any] = {}
    ...
    def is_available(cls) -> bool:
        return (
            PandaPowerSolver.is_available()
            and importlib.util.find_spec("andes") is not None
            and (
                ieee118_local_override_requested()
                or ieee118_bundled_data_available()
                or importlib.util.find_spec("powerfulcases") is not None
            )
        )
    ...
    def load_case(self, case_name: str) -> CaseInfo:
        canonical = self._normalize_case_name(case_name)
        try:
            files = self._case_files or resolve_ieee118_public_dynamic_files()
        except PublicCaseDataError as exc:
            raise ValueError(str(exc)) from exc
        try:
            andes = importlib.import_module("andes")
            interop = importlib.import_module("andes.interop.pandapower")
            to_pandapower = getattr(interop, "to_pandapower")
        except Exception as exc:
            raise SolverDependencyError(
                "ANDES and andes.interop.pandapower are required for strict "
                "IEEE118 integrated M1 conversion."
            ) from exc

        try:
            with contextlib.redirect_stdout(sys.stderr):
                system = self._load_andes_static_system(andes, files.raw_path)
                setup = getattr(system, "setup", None)
                if callable(setup):
                    setup()
                self.net = self._convert_to_pandapower(to_pandapower, system)
        except Exception as exc:
            raise ValueError(
                "Failed to convert public IEEE118 RAW data from ANDES to pandapower: "
                + str(exc)
            ) from exc

        self._case_name = canonical
        self.case_source_metadata = {
            **files.to_metadata(),
            "m1_static_conversion": "andes_to_pandapower",
            "m1_static_conversion_verify": False,
        }
        return self.case_info()

    def run_powerflow(self) -> Dict[str, Any]:
        """Run a robust pandapower solve for the RAW-derived IEEE118 net.

        The public IEEE118 RAW-derived pandapower network can be harder to solve
        than pandapower's built-in ``case118``.  Use a small retry ladder before
        declaring the strict M1 conversion unusable.
        """

        self._require_loaded()
        original_net = copy.deepcopy(self.net)
        errors: List[str] = []
        for label, options in self._runpp_attempts():
            self.net = copy.deepcopy(original_net)
            try:
                self._pp.runpp(self.net, numba=self._use_numba, **options)
            except Exception as exc:
                errors.append(f"{label}: {exc}")
                continue
            if bool(getattr(self.net, "converged", False)):
                if self.case_source_metadata is not None:
                    self.case_source_metadata["m1_powerflow_attempt"] = label
                    self.case_source_metadata["m1_powerflow_options"] = dict(options)
                return self._summary()
            errors.append(f"{label}: pandapower did not converge")

        self.net = original_net
        raise PowerFlowError(
            "RAW-derived IEEE118 pandapower power flow failed after "
            f"{len(errors)} attempt(s): " + "; ".join(errors)
        )

    @staticmethod
    def _convert_to_pandapower(to_pandapower: Any, system: Any) -> Any:
        try:
            return to_pandapower(system, verify=False)
        except TypeError:
            return to_pandapower(system)

    @staticmethod
    def _runpp_attempts() -> List[tuple[str, Dict[str, Any]]]:
        return [
            (
                "nr_dc_50",
                {
    ...
    @staticmethod
    def _load_andes_static_system(andes: Any, raw_path: str) -> Any:
        load = getattr(andes, "load", None)
        if not callable(load):
            raise SolverDependencyError("ANDES does not expose load()")
        try:
            return load(
                raw_path,
                setup=False,
            ...
            return load(raw_path, setup=False)
