...
class GridMindAgent:
        ...
        *,
        ...
        if self.config.enable_forced_capacity_routing:
            ...
            if forced is not None:
                return forced
        limitation = self._handle_transient_interconnection_limitation(
            user_message,
            context=context,
        )
    ...
    def _handle_transient_interconnection_limitation(
        self,
        user_message: str,
        *,
        context: Optional[Mapping[str, Any]],
        ...
        if not decision.should_limit:
            return None

        raw_text = str(decision.message or "M2 dynamic interconnection modeling is not implemented.")
        grounding = validate_grounding(raw_text, invoked_tools=[])
        readiness_check = {
            "type": "transient_interconnection_limitation",
            "decision": decision.to_dict(),
        }
        return AgentTurnResult(
            status="completed",
