...
def build_deterministic_report(results: Sequence[Mapping[str, Any]]) -> DeterministicReport:
    ...
    if tool == "run_transient_stability":
        return _transient_report(primary)
    if tool == "run_emt_screening":
        return _emt_report(primary)
    if tool == "run_cia":
        return _cia_report(primary)
    if tool == "run_integrated_assessment":
        return _integrated_assessment_report(primary)
    if tool == "list_real_psse_cases":
        return _real_psse_case_list_report(primary)
    if tool == "run_real_psse_assessment":
        return _real_psse_assessment_report(primary)
    if tool == "list_real_interconnection_actions":
        return _real_interconnection_action_list_report(primary)
    if tool == "run_real_interconnection_assessment":
        return _real_interconnection_assessment_report(primary)
    if tool == "list_remote_psse_m1m2_cases":
        return _remote_psse_case_list_report(primary)
    if tool == "run_remote_psse_m1m2":
        return _remote_psse_m1m2_report(primary)
...
def _error_report(result: Mapping[str, Any]) -> DeterministicReport:
    ...
    error_lines = [str(item) for item in _list(execution.get("error_lines"))]
    if error_lines:
        summary += " Error lines: " + " | ".join(error_lines[:3]) + "."
...
def _transient_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    case = _value(observation.get("case"), "unknown case")
    stability = _mapping(observation.get("stability"))
    metrics = _mapping(observation.get("metrics"))
    disturbance = _mapping(observation.get("disturbance"))
    connection_application = _mapping(observation.get("connection_application"))
    dtype = _value(disturbance.get("type"), "disturbance")
    detail = _transient_disturbance_text(disturbance)
    parts = [
        (
            f"Transient-stability report for {case}: disturbance={dtype}"
            + (f" ({detail})" if detail else "")
            + f", status={_value(stability.get('status'), 'unknown')}."
        ),
        (
            "Metrics: "
            f"tds_converged={_value(metrics.get('tds_converged'), 'unknown')}, "
            f"max_angle_spread={_format_number(metrics.get('max_angle_spread_rad'))} rad, "
            f"max_speed_deviation={_format_number(metrics.get('max_speed_deviation_pu'))} p.u., "
            f"min_voltage={_format_number(metrics.get('min_voltage_pu'))} p.u."
        ),
    ]
    if connection_application.get("applied") is True:
        parts.append(
            "Connection model: static PQ "
            f"mode={_value(connection_application.get('mode'), 'unknown')}, "
            f"p0={_format_number(connection_application.get('p0_pu'))} p.u., "
            f"q0={_format_number(connection_application.get('q0_pu'))} p.u."
        )
    reason_codes = _list(metrics.get("reason_codes"))
    if reason_codes:
        parts.append("Reason codes: " + ", ".join(str(code) for code in reason_codes) + ".")
    limitations = []
    if stability.get("status") == "borderline":
        limitations.append("incomplete_dynamic_metrics")
    return DeterministicReport(
        available=True,
        source_tool="run_transient_stability",
        summary_text=" ".join(parts),
        facts=observation,
...
def _emt_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    ...
    parts = [
        (
            f"EMT/SCR screening report for {p_mw} MW {connection_type} "
            f"at bus {bus} on {case}: status={_value(observation.get('status'), 'unknown')}."
        ),
        (
            "Metrics: "
            f"SCR={_format_number(metrics.get('scr'))}, "
            f"short_circuit_strength={_format_number(metrics.get('short_circuit_mva'))} MVA, "
            f"project_size={_format_number(metrics.get('project_mva'))} MVA, "
            f"threshold={_format_number(metrics.get('threshold'))}."
        ),
    ]
    reason_codes = _list(metrics.get("reason_codes"))
    if reason_codes:
        parts.append("Reason codes: " + ", ".join(str(code) for code in reason_codes) + ".")
    limitations = _list(observation.get("limitations"))
    if limitations:
        parts.append("Limitations: " + ", ".join(str(item) for item in limitations) + ".")
    return DeterministicReport(
        available=True,
        source_tool="run_emt_screening",
        summary_text=" ".join(parts),
        facts=observation,
...
def _integrated_assessment_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    connection = _mapping(observation.get("connection"))
    summary = _mapping(observation.get("summary"))
    case = _value(observation.get("case"), "unknown case")
    ...
    bus = _value(connection.get("bus"), "unknown bus")
    recommendation = _value(observation.get("recommendation"), "unknown")
    complete = _value(observation.get("complete"), "unknown")
    parts = [
        (
            f"Integrated M1+M2 assessment for {p_mw} MW {connection_type} "
            f"at bus {bus} on {case}: recommendation={recommendation}, complete={complete}."
        ),
        (
            "Stage summary: "
            f"M1={_value(summary.get('m1_recommendation'), 'unknown')}, "
            f"M2={_value(summary.get('m2_status'), 'unknown')}, "
            f"M2 stability={_value(summary.get('m2_stability_status'), 'unknown')}, "
            f"EMT={_value(summary.get('emt_status'), 'unknown')}, "
            f"SCR={_format_number(summary.get('emt_scr'))}."
        ),
    ]
    stage_text = _stage_status_text(_list(observation.get("stage_statuses")))
    if stage_text:
    ...
    if limitations:
        parts.append("Limitations: " + ", ".join(str(item) for item in limitations) + ".")
    report_limitations = [str(item) for item in limitations]
    if observation.get("complete") is False:
        report_limitations.append("incomplete_assessment")
    return DeterministicReport(
        available=True,
        source_tool="run_integrated_assessment",
        summary_text=" ".join(parts),
        facts=observation,
...
def _real_psse_case_list_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    cases = _list(observation.get("cases"))
    available = sum(1 for case in cases if _mapping(case).get("available") is True)
    case_ids = [
        str(_mapping(case).get("case_id"))
        for case in cases
        if _mapping(case).get("case_id")
    ]
    parts = [
        (
            "Frozen PSS/E real-data cases: "
            f"{available}/{len(cases)} available from {observation.get('processed_dir')}."
        )
    ]
    if case_ids:
        parts.append("Case ids: " + ", ".join(case_ids) + ".")
    psse_version = observation.get("psse_version")
    if psse_version:
        parts.append(f"PSS/E version: {psse_version}.")
    limitations = _list(observation.get("limitations"))
    return DeterministicReport(
        available=True,
        source_tool="list_real_psse_cases",
        summary_text=" ".join(parts),
        facts=observation,
        limitations=[str(item) for item in limitations],
    )
...
def _real_psse_assessment_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    summary = _mapping(observation.get("summary"))
    case_id = _value(observation.get("case_id"), "unknown real-data case")
    recommendation = _value(observation.get("recommendation"), "unknown")
    complete = _value(observation.get("complete"), "unknown")
    parts = [
        (
            f"Frozen PSS/E assessment for {case_id}: "
            f"recommendation={recommendation}, complete={complete}."
        ),
        (
            "Stage summary: "
            f"M1={_value(summary.get('m1_status'), 'unknown')}, "
            f"M2={_value(summary.get('m2_status'), 'unknown')}, "
            f"EMT={_value(summary.get('emt_status'), 'unknown')}."
        ),
        (
            "POC metrics: "
            f"M1 P={_format_number(summary.get('m1_poc_p_mw'))} MW, "
            f"M1 Q={_format_number(summary.get('m1_poc_q_mvar'))} MVAr, "
            f"M2 final P={_format_number(summary.get('m2_final_poc_p_mw'))} MW, "
            f"M2 final Q={_format_number(summary.get('m2_final_poc_q_mvar'))} MVAr."
        ),
    ]
    psse_version = observation.get("psse_version")
    if psse_version:
        parts.append(f"Backend: frozen PSS/E RMS results from {psse_version}.")
    stage_text = _stage_status_text(_list(observation.get("stage_statuses")))
    if stage_text:
        parts.append("Stage status: " + stage_text + ".")
    limitations = [str(item) for item in _list(observation.get("limitations"))]
    if limitations:
    ...
    return DeterministicReport(
        available=True,
        source_tool="run_real_psse_assessment",
        summary_text=" ".join(parts),
        facts=observation,
        limitations=list(dict.fromkeys(report_limitations)),
    )
...
def _remote_psse_case_list_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    cases = _list(observation.get("cases"))
    case_text = ", ".join(
        str(_mapping(item).get("case_id"))
        for item in cases
        if _mapping(item).get("case_id")
    )
    remote_configured = _value(observation.get("remote_configured"), "unknown")
    parts = [
        (
            "Remote PSS/E M1+M2 gym cases: "
            f"{case_text or 'none listed'}; remote_configured={remote_configured}."
        )
    ]
    ...
    if health.get("checked"):
        parts.append(
            "Health check: "
            f"ok={_value(health.get('ok'), 'unknown')}, "
            f"service={_value(health.get('service'), 'unknown')}."
        )
    limitations = [str(item) for item in _list(observation.get("limitations"))]
    if limitations:
        parts.append("Limitations: " + ", ".join(limitations) + ".")
    return DeterministicReport(
        available=True,
        source_tool="list_remote_psse_m1m2_cases",
        summary_text=" ".join(parts),
        facts=observation,
        limitations=limitations,
    )
...
def _remote_psse_m1m2_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    summary = _mapping(observation.get("summary"))
    case_id = _value(observation.get("case_id"), "unknown remote case")
    scenario_type = _value(observation.get("scenario_type"), "unknown scenario")
    recommendation = _value(observation.get("recommendation"), "unknown")
    complete = _value(observation.get("complete"), "unknown")
    parts = [
        (
            f"Remote PSS/E M1+M2 assessment for {case_id}/{scenario_type}: "
            f"recommendation={recommendation}, complete={complete}."
        ),
        (
            "Stage summary: "
    ...
    if summary.get("m2_final_time_s") is not None:
        parts.append(
            "Dynamic run: "
            f"final_time={_format_number(summary.get('m2_final_time_s'))} s, "
            f"rows={_value(summary.get('m2_channel_row_count'), 'unknown')}."
        )
    psse_version = observation.get("psse_version")
    if psse_version:
        parts.append(f"Backend: live remote PSS/E RMS worker from {psse_version}.")
    stage_text = _stage_status_text(_list(observation.get("stage_statuses")))
    if stage_text:
        parts.append("Stage status: " + stage_text + ".")
    limitations = [str(item) for item in _list(observation.get("limitations"))]
    ...
    return DeterministicReport(
        available=True,
        source_tool="run_remote_psse_m1m2",
        summary_text=" ".join(parts),
        facts=observation,
        limitations=limitations,
    )
...
def _real_interconnection_action_list_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    buses = _list(observation.get("connection_buses"))
    bus_ids = [
        str(_mapping(bus).get("bus"))
        for bus in buses
        if _mapping(bus).get("bus") is not None
    ]
    parts = [
        (
            f"Frozen PSS/E interconnection action space for "
            f"{_value(observation.get('case_id'), 'unknown case')}: "
            f"catalog={_value(observation.get('catalog_scenario_count'), 0)} scenarios, "
            f"results={_value(observation.get('precomputed_result_count'), 0)}."
        )
    ]
    if bus_ids:
        parts.append("Candidate buses: " + ", ".join(bus_ids) + ".")
    parts.append(
        f"Disturbance count={_value(observation.get('disturbance_count'), 'unknown')}, "
        f"estimated grid size={_value(observation.get('scenario_count_estimate'), 'unknown')}."
    )
    limitations = [str(item) for item in _list(observation.get("limitations"))]
    if limitations:
        parts.append("Limitations: " + ", ".join(limitations) + ".")
    return DeterministicReport(
        available=True,
        source_tool="list_real_interconnection_actions",
        summary_text=" ".join(parts),
        facts=observation,
...
def _real_interconnection_assessment_report(result: Mapping[str, Any]) -> DeterministicReport:
    observation = build_tool_observation(result)
    summary = _mapping(observation.get("summary"))
    connection = _mapping(observation.get("connection"))
    disturbance = _mapping(observation.get("disturbance"))
    ...
    parts = [
        (
            f"Frozen PSS/E interconnection assessment for {case_id}: "
            f"recommendation={_value(observation.get('recommendation'), 'unknown')}, "
            f"complete={_value(observation.get('complete'), 'unknown')}."
        ),
        (
            "Connection: "
            f"type={_value(connection.get('connection_type'), 'unknown')}, "
            f"bus={_value(connection.get('bus'), 'unknown')}, "
            f"P={_format_number(connection.get('p_mw'))} MW, "
            f"Q={_format_number(connection.get('q_mvar'))} MVAr."
        ),
        (
            "Disturbance: "
            f"type={_value(disturbance.get('type'), 'unknown')}, "
            f"fault_bus={_value(disturbance.get('bus'), 'n/a')}, "
            f"clearing={_format_number(disturbance.get('clearing_time_s'))} s."
        ),
        (
            "Stage summary: "
            f"M1={_value(summary.get('m1_status'), 'unknown')}, "
            f"M2 RMS={_value(summary.get('m2_status'), 'unknown')}, "
            f"SCR/EMT={_value(summary.get('scr_or_emt_status'), 'unknown')}."
        ),
    ]
    stage_text = _stage_status_text(_list(observation.get("stage_statuses")))
    if stage_text:
    ...
    return DeterministicReport(
        available=True,
        source_tool="run_real_interconnection_assessment",
        summary_text=" ".join(parts),
        facts=observation,
        limitations=list(dict.fromkeys(report_limitations)),
    )
...
def _transient_disturbance_text(disturbance: Mapping[str, Any]) -> str:
    dtype = disturbance.get("type")
    if dtype == "bus_fault":
        bus = _optional_text(disturbance.get("bus"))
        start = _optional_text(disturbance.get("fault_start_s"))
        clear = _optional_text(disturbance.get("clearing_time_s"))
        bits = []
        if bus:
            bits.append(f"bus={bus}")
        if start:
            bits.append(f"start={start}s")
        if clear:
            bits.append(f"clear={clear}s")
        return ", ".join(bits)
    if dtype == "line_trip":
        model = _optional_text(disturbance.get("model"))
        device = _optional_text(disturbance.get("device"))
        trip = _optional_text(disturbance.get("trip_time_s"))
        bits = []
        if model:
