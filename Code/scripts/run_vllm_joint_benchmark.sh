...
usage() {
  cat <<'EOF'
Launch vLLM, run the true joint M1+M2(+EMT) benchmark, save results, and stop vLLM.
...
Common options:
  ...
  --benchmark-model NAME         Model name passed to run_joint_benchmark.py. Defaults to served name.
  ...
  --connect-host HOST            Host used by benchmark client. Default: 127.0.0.1.
...
Benchmark options:
  --live-oracle                  Execute real integrated M1+M2(+EMT) oracle tools during grading.
  ...
  --skip-ieee118-preflight       Do not fail fast when an IEEE118 live run is
                                  missing public RAW/DYR data.
  --                             Remaining arguments are forwarded to run_joint_benchmark.py.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --keep-server)
      ...
      shift
      ;;
    --skip-ieee118-preflight)
      SKIP_IEEE118_PREFLIGHT=1
      shift
      ;;
    -h|--help)
      usage
...
RUN_DIR="${OUTPUT_DIR}/${TIMESTAMP}_${RUN_NAME}_joint_port${PORT}"
VLLM_LOG="${RUN_DIR}/vllm.log"
BENCH_STDERR="${RUN_DIR}/joint_benchmark.stderr.log"
RESULT_JSON="${RUN_DIR}/joint_result.json"
META_JSON="${RUN_DIR}/run_metadata.json"
IEEE118_PREFLIGHT_JSON="${RUN_DIR}/ieee118_preflight.json"
IEEE118_PREFLIGHT_STDERR="${RUN_DIR}/ieee118_preflight.stderr.log"
SELECTED_SCENARIOS_JSON="${RUN_DIR}/selected_scenarios.json"
mkdir -p "$RUN_DIR"
...
BENCH_ARGS=(
  python3 scripts/run_joint_benchmark.py
  --base-url "http://${CONNECT_HOST}:${PORT}/v1"
  --model "$BENCHMARK_MODEL"
)
if [[ "$LIVE_ORACLE" == "1" ]]; then
  BENCH_ARGS+=(--live-oracle)
fi
...
log "Live oracle: ${LIVE_ORACLE}"
log "GPUs: ${CUDA_VISIBLE_DEVICES}; TP size: ${TP_SIZE}; port: ${PORT}"
if [[ -n "${CUDA_HOME:-}" ]]; then
  log "CUDA_HOME: ${CUDA_HOME}"
fi

if [[ "$LIVE_ORACLE" == "1" && "$SKIP_IEEE118_PREFLIGHT" != "1" ]]; then
  (
    cd "$CODE_DIR"
    python3 scripts/run_joint_benchmark.py --list-scenarios "${BENCH_EXTRA_ARGS[@]}"
  ) >"$SELECTED_SCENARIOS_JSON"
  SHOULD_PREFLIGHT_IEEE118="$(
    python3 - "$SELECTED_SCENARIOS_JSON" <<'PY'
import json
import sys
...
for scenario in scenarios:
    ...
    if "ieee118" in scenario_id or any("ieee118" in tag for tag in tags):
        print("1")
        break
else:
    print("0")
PY
  ...
  if [[ "$SHOULD_PREFLIGHT_IEEE118" == "1" ]]; then
    log "Running IEEE118 RAW/DYR preflight; result: ${IEEE118_PREFLIGHT_JSON}"
    set +e
    (
      cd "$CODE_DIR"
      python3 scripts/check_ieee118_dynamic_data.py --runtime
    ) >"$IEEE118_PREFLIGHT_JSON" 2>"$IEEE118_PREFLIGHT_STDERR"
    PREFLIGHT_EXIT=$?
    set -e
    if [[ "$PREFLIGHT_EXIT" -ne 0 ]]; then
      log "IEEE118 public RAW/DYR preflight failed. vLLM was not started."
      cat "$IEEE118_PREFLIGHT_JSON" >&2 || true
      if [[ -s "$IEEE118_PREFLIGHT_STDERR" ]]; then
        log "IEEE118 preflight stderr:"
        tail -80 "$IEEE118_PREFLIGHT_STDERR" >&2 || true
      fi
      log "Use --skip-ieee118-preflight only if you intentionally want to test routing despite missing IEEE118 DYR data."
      exit "$PREFLIGHT_EXIT"
    fi
  fi
fi
...
log "Running joint benchmark; result: ${RESULT_JSON}"
set +e
(
  cd "$CODE_DIR"
  "${BENCH_ARGS[@]}"
...
export LIVE_ORACLE_VALUE="$LIVE_ORACLE"
...
export IEEE118_PREFLIGHT_JSON_PATH="$IEEE118_PREFLIGHT_JSON"
export IEEE118_PREFLIGHT_STDERR_PATH="$IEEE118_PREFLIGHT_STDERR"
export SELECTED_SCENARIOS_JSON_PATH="$SELECTED_SCENARIOS_JSON"
...
suite = result.get("suite") if isinstance(result, dict) else None
metadata = {
    ...
    "live_oracle": os.environ["LIVE_ORACLE_VALUE"] == "1",
    ...
    "benchmark_total": suite.get("total") if isinstance(suite, dict) else None,
    "benchmark_passed": suite.get("passed") if isinstance(suite, dict) else None,
    "benchmark_failed": suite.get("failed") if isinstance(suite, dict) else None,
    "benchmark_duration_s": suite.get("duration_s") if isinstance(suite, dict) else None,
    "files": {
        "result_json": result_path,
        "benchmark_stderr": os.environ["BENCH_STDERR_PATH"],
        "vllm_log": os.environ["VLLM_LOG_PATH"],
        "ieee118_preflight_json": os.environ["IEEE118_PREFLIGHT_JSON_PATH"],
        "ieee118_preflight_stderr": os.environ["IEEE118_PREFLIGHT_STDERR_PATH"],
        "selected_scenarios_json": os.environ["SELECTED_SCENARIOS_JSON_PATH"],
    },
}
