...
usage() {
  cat <<'EOF'
Launch vLLM, run the integrated M1+M2 benchmark, save results, and stop vLLM.
...
Common options:
  ...
  --benchmark-model NAME         Model name passed to run_integrated_benchmark.py. Defaults to served name.
  --suite all|m1|m2              Which benchmark suite(s) to run. Default: all.
...
Benchmark options:
  --live-m2-oracle               Execute real ANDES M2 oracle tools during grading.
  ...
  --                             Remaining arguments are forwarded to run_integrated_benchmark.py.
...
case "$SUITE" in
  all|m1|m2) ;;
  *) die "--suite must be one of: all, m1, m2" ;;
esac
...
RUN_DIR="${OUTPUT_DIR}/${TIMESTAMP}_${RUN_NAME}_integrated_${SUITE}_port${PORT}"
VLLM_LOG="${RUN_DIR}/vllm.log"
BENCH_STDERR="${RUN_DIR}/integrated_benchmark.stderr.log"
RESULT_JSON="${RUN_DIR}/integrated_result.json"
...
BENCH_ARGS=(
  python3 scripts/run_integrated_benchmark.py
  --suite "$SUITE"
  --base-url "http://${CONNECT_HOST}:${PORT}/v1"
  --model "$BENCHMARK_MODEL"
)
if [[ "$LIVE_M2_ORACLE" == "1" ]]; then
  BENCH_ARGS+=(--live-m2-oracle)
fi
...
log "Suite: ${SUITE}; live M2 oracle: ${LIVE_M2_ORACLE}"
...
log "Running integrated benchmark; result: ${RESULT_JSON}"
set +e
(
  cd "$CODE_DIR"
  "${BENCH_ARGS[@]}"
...
export SUITE_VALUE="$SUITE"
export LIVE_M2_ORACLE_VALUE="$LIVE_M2_ORACLE"
...
metadata = {
    ...
    "suite": os.environ["SUITE_VALUE"],
    "live_m2_oracle": os.environ["LIVE_M2_ORACLE_VALUE"] == "1",
    ...
    "benchmark_total": result.get("total") if isinstance(result, dict) else None,
    "benchmark_passed": result.get("passed") if isinstance(result, dict) else None,
    "benchmark_failed": result.get("failed") if isinstance(result, dict) else None,
    "benchmark_duration_s": result.get("duration_s") if isinstance(result, dict) else None,
    "m1": result.get("m1") if isinstance(result, dict) else None,
    "m2": result.get("m2") if isinstance(result, dict) else None,
    "files": {
