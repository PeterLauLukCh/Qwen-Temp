#!/usr/bin/env python3
"""Run the frozen real-data PSS/E interconnection benchmark."""
...
from gridmind_mini.real_interconnection_benchmark import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the frozen real-data PSS/E interconnection benchmark."
    )
    ...
    parser.add_argument("--processed-dir", help="Frozen interconnection directory.")
    ...
    scenarios = filter_real_interconnection_scenarios(
        default_real_interconnection_benchmark_scenarios(processed_dir=args.processed_dir),
    ...
    if args.oracle_only:
        outputs = run_real_interconnection_oracles(scenarios, oracle_registry)
    ...
    runner = RealInterconnectionBenchmarkRunner(agent, oracle_registry)
    try:
...
def _oracle_summary(item: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "scenario_result_id": item.get("scenario_result_id"),
